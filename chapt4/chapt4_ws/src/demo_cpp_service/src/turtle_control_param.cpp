...
public:
    explicit TurtleControlNode(const std::string& node_name): Node(node_name)
    {
            ...
            [&](const std::vector<rclcpp::Parameter> & parameters) -> SetParametersResult
            {
                ...
                for (const auto& parameter : parameters)
                {
                    if (parameter.get_name() == "k")
                    {
                        ...
                        RCLCPP_INFO(this->get_logger(), "已更新参数 k 为: %.2f", k_);
                    }
                    else if (parameter.get_name() == "max_speed")
                    {
                        max_speed_ = parameter.as_double();
                        RCLCPP_INFO(this->get_logger(), "已更新参数 max_speed 为: %.2f", max_speed_);
                    }
                }
        ...
        subscriber_ = this->create_subscription<turtlesim::msg::Pose>("/turtle1/pose", 
            10, std::bind(&TurtleControlNode::on_pose_received, this, std::placeholders::_1));
    }
    ...
    void on_pose_received(const turtlesim::msg::Pose::SharedPtr pose)
    {
        ...
        RCLCPP_INFO(get_logger(), "当前坐标: (%.2f, %.2f)", current_x, current_y);

        // 计算到目标点的距离
        auto error_x = target_x_ - current_x;
        auto error_y = target_y_ - current_y;
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<TurtleControlNode>("turtle_controller");
    rclcpp::spin(node);
    rclcpp::shutdown();
